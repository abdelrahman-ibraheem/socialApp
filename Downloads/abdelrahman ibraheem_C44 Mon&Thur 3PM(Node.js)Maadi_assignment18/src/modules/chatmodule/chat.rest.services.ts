...
export class ChatServices {
  ...
  createChatGroup = async (req: Request, res: Response): Promise<void> => {
...
const newGroup = await this.userModel.create({
  data:{
    participants: [ ...dbParticipants.map((p) => p._id), user._id],
    group:true,
    roomId,
    createdBy: user._id,
  }
...
if(!newGroup){
  throw new ApplicationException("fail to create group",500)}
      successHandler({res, data: newGroup,  });
    }
    getChatGroup = async (req: Request, res: Response): Promise<void> => {
